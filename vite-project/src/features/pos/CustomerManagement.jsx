      ...
      <div className="max-w-5xl mx-auto">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 mb-6">
          <div>
            ...
            <p className="text-sm text-slate-500">Manage your retail customers</p>
          </div>
          <button
            onClick={openCreate}
                ...
                <tbody className="divide-y divide-slate-100">
                  {customers.map((c) => (
                    <tr key={c._id} className="hover:bg-slate-50 transition-colors">
                      ...
                      <td className="px-4 py-3 text-slate-600 max-w-[200px] truncate">{c.address || "-"}</td>
                      <td className="px-4 py-3 text-right">
                        <span className={`font-medium ${c.creditBalance > 0 ? "text-red-600" : "text-slate-600"}`}>
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              ...
              <div>
                ...
                <textarea value={form.address} onChange={(e) => setForm({ ...form, address: e.target.value })}
                  rows={2} className="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-slate-400" />
              </div>
