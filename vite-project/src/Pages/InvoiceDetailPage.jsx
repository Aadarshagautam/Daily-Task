...
const InvoiceDetailPage = () => {
  ...
  const statusConfig = {
    draft: { color: 'bg-slate-100 text-slate-700 border-slate-300', icon: Clock },
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="space-y-2">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="text-sm font-medium text-slate-900 capitalize">{invoice.paymentMethod?.replace('_', ' ')}</span>
                </div>
                {invoice.withoutVat && (
                  <div className="flex justify-between">
                ...
                <div className="flex justify-between">
                  <span className="text-sm text-slate-600">Calc Mode:</span>
                  <span className="text-sm font-medium text-slate-900">{vatDiscountModeLabel}</span>
                </div>
              </div>
            </div>
          </div>
              ...
              <table className="w-full">
                <thead className="bg-slate-50 border-b border-slate-200">
                  <tr>
                    ...
                    <th className="px-6 py-3 text-center text-xs font-medium text-slate-500 uppercase">VAT%</th>
                ...
                <tbody className="divide-y divide-slate-200">
                  {(invoice.items || []).map((item, index) => (
                    <tr key={item._id || index} className="hover:bg-slate-50">
                      ...
                      <td className="px-6 py-4 text-center text-sm text-slate-900">{item.quantity}</td>
                      <td className="px-6 py-4 text-right text-sm text-slate-900">{'\u20B9'}{item.unitPrice.toLocaleString('en-IN')}</td>
                      <td className="px-6 py-4 text-center text-sm text-slate-600">{invoice.withoutVat ? '0' : item.vatRate}%</td>
                      <td className="px-6 py-4 text-right text-sm text-slate-600">
                        {item.discountAmount > 0 ? `\u20B9${item.discountAmount.toLocaleString('en-IN')}` : '-'}
                      </td>
                      <td className="px-6 py-4 text-right text-sm font-semibold text-slate-900">{'\u20B9'}{item.lineTotal.toLocaleString('en-IN')}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          <div className="bg-white rounded-xl border border-slate-200 p-6 sticky top-24">
            ...
            <div className="space-y-3">
              <div className="flex justify-between text-sm">
                ...
                <span className="font-medium text-slate-900">{'\u20B9'}{invoice.subtotal.toLocaleString('en-IN')}</span>
              </div>
              ...
              {!invoice.withoutVat && invoice.totalVat > 0 && (
                <div className="flex justify-between text-sm">
                  ...
                  <span className="font-medium text-slate-900">{'\u20B9'}{invoice.totalVat.toLocaleString('en-IN')}</span>
                </div>
              )}
              ...
              <div className="border-t border-slate-200 pt-3 mt-3">
                <div className="flex justify-between">
                  ...
                  <span className="text-2xl font-bold text-slate-600">{'\u20B9'}{invoice.grandTotal.toLocaleString('en-IN')}</span>
                </div>
              </div>
            </div>
            ...
            <div className="mt-6 space-y-2">
              {invoice.status === 'draft' && (
                <button
                  onClick={() => updateStatus('sent')}
                  className="w-full flex items-center justify-center gap-2 px-4 py-2 bg-slate-900 hover:bg-slate-800 text-white rounded-lg transition-colors"
                >
              ...
              <button
                onClick={downloadPDF}
                className="w-full flex items-center justify-center gap-2 px-4 py-2 bg-white border border-slate-300 text-slate-700 rounded-lg hover:bg-slate-50 transition-colors"
              >
